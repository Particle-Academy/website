...
### Changed
- **`@particle-academy/react-fancy` 5.15.0 → 5.26.0.** The one breaking entry in that span is 5.17.0's "`Field` no longer spaces itself"; every place this site stacks two or more `Field`s already sits in a `space-y-4` form or a `grid gap-4` row, so no markup changed. The update also drops `clsx` from the installed tree (5.22.0 inlined it).
