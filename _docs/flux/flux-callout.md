# Flux Callout Component

The Callout component is designed to highlight important information or guide users toward key actions within your application.

## Basic Usage

The basic callout component includes an icon, heading, and text content:
...
For a more compact layout, place the icon inside the heading by adding the icon prop to `flux:callout.heading` instead of the root component:
...
<flux:callout icon="clock">
    ...
    <flux:callout.text>
        Your current plan will expire in 3 days. Renew now to avoid service interruption 
        and continue accessing premium features.
    </flux:callout.text>
    <x-slot name="actions">
...
Add a close button using the `controls` slot to allow users to dismiss callouts:

> Note: Callouts do not include built-in dismiss functionality. This is intentional—dismissal behavior depends on your app's needs. You might want to remove it from the DOM on the frontend or persist the dismissal in a backend database.
...
<flux:callout icon="finger-print" variant="secondary">
    ...
    <flux:callout.text>
        We detected a login from a new device in <span class="font-medium text-zinc-800 dark:text-white">New York, USA</span>. 
...
<flux:callout variant="warning" icon="exclamation-circle" 
    heading="Please verify your account to unlock all features." />

<flux:callout variant="danger" icon="x-circle" 
    heading="Something went wrong. Try again or contact support." />
```
...
<flux:callout variant="secondary" icon="user-group">
    ...
    <flux:callout.text>
        <p>Share projects, manage permissions, and collaborate in real time with your team. 
        Upgrade now to access these features.</p>
    </flux:callout.text>
    <x-slot name="actions">
...
| Prop         | Description                                                                                                           |
| ------------ | --------------------------------------------------------------------------------------------------------------------- |
| icon         | Name of the icon displayed next to the heading (e.g., clock). See Flux Icon documentation for available icons.        |
| icon-variant | Variant of the icon displayed next to the heading (e.g., outline). See Flux Icon documentation for available variants.|
| variant      | Options: secondary, success, warning, danger. Default: secondary                                                      |
| color        | Custom color (e.g., red, blue). Uses Tailwind color palette.                                                          |
